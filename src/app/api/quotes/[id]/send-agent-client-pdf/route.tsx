...
export async function POST(
  ...
  try {
    ...
    if (!quote.agentClientPdfUrl) {
      return NextResponse.json(
        { ok: false, error: "Agent PDF not generated." },
        { status: 400 }
      );
    }

    const absoluteUrl = toAbsoluteUrl(quote.agentClientPdfUrl);

    if (!absoluteUrl) {
      return NextResponse.json(
        { ok: false, error: "PDF URL is invalid." },
        { status: 400 }
      );
    }
    ...
    if (!pdfResponse.ok) {
      return NextResponse.json(
        {
          ok: false,
          error: `Could not load PDF. Status: ${pdfResponse.status}`,
          pdfUrl: absoluteUrl,
        },
        { status: 500 }
      );
    ...
    console.error("SEND AGENT PDF ERROR:", err);
