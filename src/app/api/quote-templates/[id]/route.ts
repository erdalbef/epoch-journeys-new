...
import { quoteEmailTemplate } from "@/lib/email/templates/quoteEmail"
  ...
  try {
    ...
    if (!quote.pdfUrl) {
      return NextResponse.json(
        {
          message:
            "Quote PDF is missing. Finalize the quote again before sending.",
        },
        { status: 400 }
      )
    }

    const quoteReference = String(quote.quoteReference || quote.quoteNumber)

    await sendQuoteEmail({
      to: quote.recipientEmail,
      subject: `Quote ${quoteReference}`,
    ...
    if (!updated) {
      return NextResponse.json(
        { message: "Quote sent but could not be reloaded." },
        { status: 500 }
      )
    }
