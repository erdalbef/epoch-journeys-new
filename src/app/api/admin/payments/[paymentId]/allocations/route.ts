...
export async function POST(
  ...
  try {
    ...
    const originalSafeFileName =
      safeFileName(
        uploaded.name,
      );

    const preferredFileName =
      `customer-payment-${payment.id}-${originalSafeFileName}`;
     ...
     * ==========================================================
     *
     * All finance/accounting uploads must go through
     * saveFinanceFile().
     *
     * Production:
     *   Private Vercel Blob
     ...
     *
     * This prevents finance routes from independently creating
     * Blob pathname records or writing to Vercel's read-only
     * filesystem.
     * ==========================================================
     */
     ...
     *
     * Only now remove the previous proof so a failed database
     * transaction can never destroy the existing document.
     * ==========================================================
     */
    ...
    /*
     * The new file is now owned by the FinanceDocument.
     * Do not remove it in the catch block.
     */
    ...
    console.error(
      "Customer payment proof upload failed:",
      error,
    );
