...
import { ResourceAudience, ResourceStatus } from "@prisma/client";
...
export default function ResourceEditForm({
  ...
  const [audience, setAudience] = useState<ResourceAudience>(resource.audience);
  const [categoryId, setCategoryId] = useState(resource.categoryId || "");
  const [tourId, setTourId] = useState(resource.tourId || "");
  const [destinations, setDestinations] = useState(resource.destinations.join(", "));
  ...
  function changeAudience(next: ResourceAudience) {
    ...
    if (!currentCategory || currentCategory.audience !== next) {
      setCategoryId("");
    }
  }
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data?.error || "Could not update resource.");
        return;
      }

      router.push(`/admin/resources?audience=${audience}&status=${resource.status}`);
      router.refresh();
    } catch {
      setError("Could not update resource. Please try again.");
    } finally {
      setBusy(false);
    }
  }
    ...
    <div className="mx-auto max-w-5xl space-y-6 p-4 sm:p-6 lg:p-8">
        ...
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">
              Update classification and details, or replace the file while keeping the same resource record.
            </p>
          </div>
          <Link
            href={`/admin/resources?audience=${resource.audience}&status=${resource.status}`}
      ...
      <form onSubmit={submit} className="space-y-6">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          <h2 className="text-lg font-bold text-[#001F3F]">1. Access</h2>
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            ...
            >
              ...
              <p className="mt-1 text-sm text-slate-500">Visible to approved agents.</p>
            </button>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-[#001F3F]">2. Details & classification</h2>
          </div>

          <div className="mt-5 grid gap-4 md:grid-cols-2">
            <label className={`${labelClass} md:col-span-2`}>
              Title *
              <input name="title" required maxLength={180} defaultValue={resource.title} className={inputClass} />
            </label>

            <label className={`${labelClass} md:col-span-2`}>
              Description
              <textarea name="description" defaultValue={resource.description || ""} className={`${inputClass} min-h-24 py-3`} />
            </label>
            ...
            <label className={labelClass}>
              Destinations
              <input value={destinations} onChange={(event) => setDestinations(event.target.value)} className={inputClass} />
              <span className="mt-1 block text-xs font-normal text-slate-400">Separate with commas.</span>
            </label>

            <label className={labelClass}>
              Tags
              <input value={tags} onChange={(event) => setTags(event.target.value)} className={inputClass} />
              <span className="mt-1 block text-xs font-normal text-slate-400">Separate with commas.</span>
            </label>

            <div className="md:col-span-2 rounded-xl border border-slate-200 bg-slate-50 p-4">
              <p className="text-sm font-semibold text-slate-700">Current file</p>
              <p className="mt-1 break-all text-sm text-slate-600">{resource.originalFileName}</p>
              <p className="mt-1 text-xs text-slate-400">{fileSize(resource.fileSize)}</p>
            </div>
              ...
              <div className="mt-1.5 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    />
                    <p className="mt-2 text-xs text-slate-500">Leave empty to keep the current file. Maximum 25 MB.</p>
                    {replacementFileName ? (
                      <p className="mt-2 text-xs font-semibold text-[#001F3F]">Replacement: {replacementFileName}</p>
                    ) : null}
                  </div>
                </div>
              </div>
            ...
            <label className="flex items-start gap-3 rounded-xl border border-slate-200 p-4 md:col-span-2">
              <input name="featured" type="checkbox" value="true" defaultChecked={resource.featured} className="mt-1 h-4 w-4" />
              <span>
                <span className="block text-sm font-semibold text-slate-800">Featured resource</span>
                <span className="mt-0.5 block text-xs leading-5 text-slate-500">Featured resources appear before normal resources.</span>
              </span>
            </label>
          </div>
        </section>
