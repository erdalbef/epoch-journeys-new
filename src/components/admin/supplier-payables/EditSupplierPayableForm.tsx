...
type Payable = {
  ...
  documents: Array<{
    ...
    createdAt: string;
    title: string;
    accountingSubcategory: string | null;
  }>;
};
...
export default function EditSupplierPayableForm({
  ...
  const originalDocumentType = payable.documentType;
  ...
  const documentTypeChanged = documentType !== originalDocumentType;
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (documentTypeChanged && !invoiceFile) {
      toast.error(
        "Please upload the new supplier document when changing the document type. The previous document will be retained in history.",
      );
      return;
    }
    ...
    try {
      ...
      toast.success(
        invoiceFile && documentTypeChanged
          ? "Supplier payable updated. New document added; previous document retained in history."
          : invoiceFile
            ? "Supplier payable updated and document replaced."
            : removeExistingDocument
              ? "Supplier payable updated and current document removed."
    ...
    >
      ...
      {documentTypeChanged ? (
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
          <strong>Document progression:</strong> the new document will become current, while the previous supplier document remains in history. Existing payments remain attached to this payable.
        </div>
      ) : null}
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Keep the current file, replace a correction, or add the next supplier document.
              When the document type changes, the previous document is retained in history.
            </p>
          </div>
        </div>
          ...
          <div className="mt-4 flex items-center justify-between gap-4 rounded-xl border bg-white p-4">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-slate-500">
                {(invoiceFile.size / 1024 / 1024).toFixed(2)} MB - {documentTypeChanged ? "new document" : "replacement file"}
              </p>
            </div>
            <button
              type="button"
      ...
      {payable.documents.length > 0 ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <p className="mt-1 text-sm text-slate-500">Newest supplier document first.</p>
          <div className="mt-4 space-y-3">
            {payable.documents.map((document, index) => (
              <div key={document.id} className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-slate-50/70 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="truncate text-sm font-semibold text-slate-900">{document.originalFileName}</p>
                    <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${index === 0 ? "bg-blue-100 text-blue-800" : "bg-slate-200 text-slate-700"}`}>
